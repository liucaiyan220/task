...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    dic={}
    set1=set()
    tup=(dic,set1) #元组格式，包括一个字典和一个集合
    for data1 in plot_countries:
          for data2 in gdp_countries.values():
			        if plot_countries[data1]==data2["Country Name"]:
				        for years in range(1960,2016):
					         if data2[str(years)]=="":
					              continue
					         else:
						          dic[data1]=data2   #把世行有GDP数据的绘图库国家信息放入字典dic中
    for data3 in plot_countries:
        if data3 not in dic:
            set1.add(data3) #把世行无GDP数据的绘图库国家代码放入集合set1中
    return tup          
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
    ...
    #访问isp_gdp.csv文件时,以gdpinfo["gdpfile"]，访问其中某属性时，以gdpinfo["Country_Name"]进行
        csvreader=csv.DictReader(csvfile,delimiter=gdpinfo["separator"],quotechar=gdpinfo["quote"])
        for row in csvreader:
            for data in plot_countries:
                country=plot_countries[data]
                if country == row[gdpinfo["country_name"]] and row[year]!="":
                    dic[data]=math.log10(float(row[year]))  
                    #将绘图库各国家代码及对应的在某具体年份GDP产值放入字典dic中
                    #将数据转换为以10为基数的对数格式
                elif country == row[gdpinfo["country_name"]] and row[year]=="":
                    set2.add(data)
                else:
                    continue
    set1=tup[1]    
    #将在世行GDP数据中完全没有记录的绘图库国家代码放入集合set1中
    set3=set2-set1 
    #将只是没有某特定年的世行GDP数据的绘图库国家代码放入集合set3中
    tup1=(dic,set1,set3)
    return tup1                
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    worldmap_chart.title = "全球GDP数据分布图"
    ...
    worldmap_chart.add("no date at this year",build_map_dict_by_name(gdpinfo, plot_countries,year)[2])
